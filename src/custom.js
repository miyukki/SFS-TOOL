...
$(function() {
	...
	Lesson.prototype.getPlanDeleteUrl = function() {
		...
		return "https://vu8.sfc.keio.ac.jp/sfc-sfs/sfs_class/student/plan_list.cgi?" + UT_getLang() + "&" + param_id + "&" + param_yc + "&mode=del";
	}
	Lesson.prototype.getSummaryUrl = function() {
		var param_id = this.getUrl().match("id=[a-z0-9]+")[0];
		var param_yc = this.getUrl().match("yc=[0-9]{4}_[0-9]+")[0];
		return "http://vu8.sfc.keio.ac.jp/course2007/summary/class_summary_by_kamoku.cgi?" + UT_getLang() + "&" + param_id + "&" + param_yc;
	}
	...
	var FC_EasyTimetable = function() {
		...
		var content = "https://vu8.sfc.keio.ac.jp/sfc-sfs/sfs_class/student/plan_timetable.cgi?" + UT_getLang() + "&" + param_term + "&" + param_id;
	...
	var FC_Homework = function() {
		...
		$.get("https://vu8.sfc.keio.ac.jp/sfc-sfs/sfs_class/student/view_timetable.cgi?"+ param_id +"&"+ param_term +"&fix=0&" + UT_getLang(), function(data) {
			...
			for (var j = 0; j < class_urls.length; j++) {
				$.ajax({type: "GET", url: class_urls[j], success: (function(url) { return function(data) {
					...
					if(remain == 0) {
						$("[src*='square_red.gif']").css({cursor: "pointer"}).click(function() {
	...
	if(location.pathname == "/sfc-sfs/sfs_class/student/s_class_top.cgi") {
		...
		var inputArea = $("<div/>").append(
			...
			$("<button/>").text("送信").click(function() {
				...
				$.post("http://d.applest.net/wellness/posts.php?" + yc + "&" + sid, {name: $(".bbsInputName").val(), body: $(".bbsInputBody").val()}, function(data) {
